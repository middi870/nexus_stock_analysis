...
@app.post("/ai/chat", summary="Proxy to Anthropic Claude API")
async def ai_chat(req: AIChatRequest):
    key = req.api_key.strip() or os.environ.get("ANTHROPIC_API_KEY", "")
    if not key:
        raise HTTPException(
            status_code=400,
            detail="No API key. Set ANTHROPIC_API_KEY env var or pass api_key in the request body."
        )
